...
; проверить установлен ли framwork на машине и какой версии. ≈сли установленна€ верси€ подходит ир _MSG="", в случае когда нужен
; framwork _MSG = сообщению
!macro CheckDotNETVersion _MSG
	...
	Pop $0
	${If} $0 == "none"
		StrCpy ${_MSG} "${PRODUCT_NAME} requires that the .NET Framework ${DOTNET_VERSION} is installed. The .NET Framework will be downloaded and installed automatically during installation of ${PRODUCT_NAME}."
		DetailPrint "${_MSG}"
	${EndIf}
	...
	${If} $1 == 2
		StrCpy ${_MSG} ".NET Framework Version found: $0, but ${PRODUCT_NAME} requires that the .NET Framework ${DOTNET_VERSION} is installed. The .NET Framework will be downloaded and installed automatically during installation of ${PRODUCT_NAME}."
		DetailPrint ${_MSG}
	${EndIf}
!macroend

; —качать и установить framwork 2.0
!macro DownloadDotNET
...
DownloadFail:
	...
	MessageBox MB_OK "The download failed with the message below.  We apologize for the inconvenience.$\n$\n$R0$\n$\nCheck your Internet connection and make sure that you are connected.  If you need help, email from_logger@mail.ru."
	Quit

DownloadCancelled:
	Delete "$TEMP\dotnetfx.exe"
	DetailPrint "Install aborted due to missing .NET Framework"
	MessageBox MB_OK "You cancelled the installation of the .NET Framework. ${PRODUCT_NAME} cannot run without the .NET Framework.  We apologize for the inconvenience.$\n$\nWe hope that you will consider trying ${PRODUCT_NAME} in the future."
	Quit
...
; запустить инстал€тор framwork. ¬ _FILE_PATH пепедать путь инсталера
!macro InstallDotNet _FILE_PATH
	DetailPrint "Pausing installation while downloaded .NET Framework installer runs."
	NsisDotNetInstaller::LaunchInstaller "${_FILE_PATH}"
	DetailPrint "Completed .NET Framework install/update. Cleaning temporary files..."
!macroend
